...
def client_question(request):
    if request.method == "POST":
        question = json.loads(request.body).get('question')
        method = json.loads(request.body).get('method')
        client_data["message"] = question
        client_data["method"] = method
        return JsonResponse({"message": "Received Successful!!!"})
    return JsonResponse({"message": "Recieve only POST method!!!"})
...
def update_learning_style(request):
    if request.method == "POST":
        learning_style['depth'] = json.loads(request.body).get('depth')
        learning_style['learning_style'] = json.loads(request.body).get('learning_style')
        learning_style['communication_style'] = json.loads(request.body).get('communication_style')
        learning_style['tone_style'] = json.loads(request.body).get('tone_style')
        learning_style['reasoning_framework'] = json.loads(request.body).get('reasoning_framework')
